...
public class PictoView extends ViewPart {
	...
	public void renderEditorContent() {
		...
		long treeViewStart = 0, treeViewEnd = 0;
		...
		try {
			...
			if (!rerender && profiling) {
				...
				for (int rep = 0; rep < numReps; rep++) {
					...
					treeViewStart = System.currentTimeMillis();
					ViewTree viewTree = source.getViewTree(editor);
					treeViewEnd = System.currentTimeMillis();
					if (rep >= notMeasuredExecutions) {
						profilingStream.println(
								String.format("%s,%d", "TreeViewer", treeViewEnd - treeViewStart));
					}
					// then, force-render all viewTrees, and measure that
					profile(viewTree, profilingStream, rep >= notMeasuredExecutions);
					System.out.println(String.format("PictoProfiler: Rep %d", rep - notMeasuredExecutions));
				}
	...
	private void profile(ViewTree viewTree, PrintStream profilingStream, boolean saveMeasurement) {
		if (viewTree.getPromise() != null) {
			long start = System.currentTimeMillis();
			viewTree.getContent().getFinal(this);
			long end = System.currentTimeMillis();
			if (saveMeasurement) {
				profilingStream.println(String.format("\"%s\",%d",
						String.join("/", viewTree.getPath()), end - start));
			}
		}
